...
// Apache License 2.0 — see LICENSE in this directory. Trimmed to the DBN v1
// record-struct layouts whose shape differs from the current (v3) structs in
// record.hpp; structs identical across versions are not re-aliased here. Member
// functions and upgrade/conversion helpers are removed; std::byte reserved
// fields are std::uint8_t. The field order, types, and the static_asserts on
// size/alignment are the wire format and must not be changed.
#pragma once
...
static constexpr std::int32_t kUndefStatQuantity = std::numeric_limits<std::int32_t>::max();

// A definition of an instrument in DBN version 1. The record of the definition
// schema.
struct InstrumentDefMsg {
	...
	std::array<std::uint8_t, 2> _reserved4 {};
	std::int64_t strike_price;
	std::array<std::uint8_t, 6> _reserved5 {};
	...
	std::array<std::uint8_t, 3> _dummy {};
};
...
// disseminated by publishers. The `stat_type` indicates the statistic contained
// in the message.
struct StatMsg {
	RecordHeader hd;
	UnixNanos ts_recv;
	...
	std::array<std::uint8_t, 6> _reserved {};
};
...
struct SymbolMappingMsg {
	...
	std::array<std::uint8_t, 4> _dummy {};
	UnixNanos start_ts;
	UnixNanos end_ts;
};
